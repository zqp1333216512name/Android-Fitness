...
public class PieChartView extends View {
    ...
    private int[] mPieColors = new int[]{Color.CYAN, Color.MAGENTA, Color.GREEN, Color.BLUE, Color.YELLOW, Color.RED};
        ...
        if (mPieItems != null && mPieItems.length > 0) {
            ...
            for (int i = 0; i < mPieItems.length; i++) {
            	...
            	if(mPieItems.length > 1 && (mPieItems.length % mPieColors.length == 1) && i == mPieItems.length - 1) {
            		piePaint.setColor(mPieColors[3]);//防止最后一个和第一个重色导致分辨不出
            	} else
